...
graphs_train = {}
with open(DATA_PATH + 'random_train.txt', 'r') as f:
    for line in f:
...
graphs_vec = {}
with open(DATA_PATH + 'content_vec_10_'+str(dime)+'.txt', 'r') as f:
    i = 0
    for line in f:
        if(i == 0):
            i+=1
            continue
        walks = line.strip().split(' ')
        graphs_vec[walks[0]] = [walks[i] for i in range(1,1+dime)]
        
average_node_vec = {}
with open(DATA_PATH + 'average_node_vec_50.txt', 'r') as f:
    i = 0
    for line in f:
        if(i == 0):
            i+=1
            continue
        walks = line.strip().split(' ')
        average_node_vec[walks[0]] = [walks[i] for i in range(1,51)]
        
contents_vec = {}
with open(DATA_PATH + 'graph_vec_10_'+str(dime)+'.txt', 'r') as f:
    i = 0
    for line in f:
        if(i == 0):
            i+=1
            continue
        walks = line.strip().split(' ')
        contents_vec[walks[0]] = [walks[i] for i in range(1,dime+1)]

graphs_val = {}
with open(DATA_PATH + 'random_val.txt', 'r') as f:
    for line in f:
        walks = line.strip().split('\t')
        graphs_val[walks[0]] = []
        for i in range(1, len(walks)):
...
graphs_test = {}
with open(DATA_PATH + 'random_test.txt', 'r') as f:
    for line in f:
        walks = line.strip().split('\t')
        graphs_test[walks[0]] = []
        for i in range(1, len(walks)):
...
for key,graph in graphs_train.items():
    ...
    temp2 = graphs_vec[key]
    temp3 = contents_vec[str(int(key)-1)]
    temp4 = average_node_vec[key]
    ave_node_vec.append(temp4)
    x_data_vec.append(temp2)
    x_data_vec_s.append(temp3)
    x_data.append(temp)
    y_data.append(np.log(y+1.0)/np.log(2.0))
    sz_data.append(sizes_train[key])
    
pickle.dump((x_data, x_data_vec,x_data_vec_s,y_data, sz_data, index.length(),ave_node_vec), open('data/data_train'+str(dime)+'.pkl','wb+'))
...
pickle.dump((x_data, x_data_vec,x_data_vec_s,y_data, sz_data, index.length(),ave_node_vec), open('data/data_val'+str(dime)+'.pkl','wb+'))
...
pickle.dump((x_data, x_data_vec,x_data_vec_s,y_data, sz_data, index.length(),ave_node_vec), open('data/data_test'+str(dime)+'.pkl','wb+'))
...
#pickle.dump(node_vec, open('data/node_vec.pkl','wb+'))