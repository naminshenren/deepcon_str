...
class DeepCon_Str(object):
    def __init__(self, config, sess, node_embed,_function):
        ...
        if(self._function=="deepcon" or self._function=="deepstr"):
            self.n_hidden_gru = 16#config.n_hidden_gru
        elif(self._function=="deepcon_str"):
...
#        train_op = optim.apply_gradients(capped_gvs)
    ...
    def get_pred(self, x,xv,xv_s, y, sz,avg):
        return self.sess.run(self.pred, feed_dict={self.x: x ,self.x_vec:xv,self.x_vec_s:xv_s,self.y: y, self.sz: sz,self.avg:avg})
    def get_predcas(self, x,xv, y, sz,avg):
        return self.sess.run(self.pred, feed_dict={self.x: x ,self.x_vec:xv,self.y: y, self.sz: sz,self.avg:avg})